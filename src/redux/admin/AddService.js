...
//   "admin/profile",
//   async (data, navigate) => {
//     console.log(data, "adservice");
//     try {
//       const response = await DataService.post(Api.PROFILE_UPDATE, data);
//       localStorage.setItem("token", response.data.data.token);
//       console.log(response.data.data.token, "token check");
...
//       // console.log(response.data.data, "adservice");
//       if (response.data.status === 200) {
//         toast.success(response.data.message);
...
export const getRoleAdmin = createAsyncThunk("admin/getRoleAdmin", async () => {
  const response = await DataService.get(Api.GET_ROLE_ADMIN);
  console.log(response, 1490);
  return response.data;
});
