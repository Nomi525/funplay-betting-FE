...
const RoleList = () => {
  ...
  useEffect(() => {
    if (!query) return setFilterData(roleList);
    setFilterData(
      roleList?.filter((item) =>
        item?.Role_type?.toLowerCase().includes(query?.toLowerCase())
      )
    );
    setPage(0);
  }, [query]);
              ...
              <Index.Box className="">
                ...
                >
                  <Index.Table
                    ...
                    className="table-design-main barge-table role-management-table"
                  >
                    <Index.TableHead>
                      <Index.TableRow>
