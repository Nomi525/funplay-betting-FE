...
const NumberBettingAllRecords = () => {
  ...
  const getAllData = async () => {
    ...
    await DataService.get(
      ...
      .then((res) => {
        setGetallGameData(res?.data?.data);
        setLoading(false);
      })
  ...
  const handleSearch = (searched) => {
    let result = getallGameData.filter(
      (item) =>
        item?.period
          ?.toString()
          .toLowerCase()
  ...
  useEffect(() => {
    setSearchedData(getallGameData);
  }, [getallGameData]);
...
export default NumberBettingAllRecords;
